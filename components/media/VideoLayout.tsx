...
// Constants
const BASE_URL = process.env.REACT_APP_BASE_URL || 'http://www.vibevision.ai';

const VideoLayout: React.FC = () => {
    // State management
    const [loading, setLoading] = useState<boolean>(true);
    const [filteredPlaylists, setFilteredPlaylists] = useState<Video[]>([]);
    ...
    useEffect(() => {
        const fetchUserContent = async () => {
            try {
                setLoading(true);
                const response = await axios.get<Video[]>(`${BASE_URL}/api/content/get-user-content`);
